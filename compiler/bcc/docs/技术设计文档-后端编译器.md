...
|------|------|--------|------|
| M0 | 范围对齐 | 边界文档（本文） | — |
| M1 | 最佳实践候选 | 候选清单 + 冲突项 | M0 |
| M2 | 最小规范定稿 | 可执行规范 v1（必须/推荐/禁用） | M1 |
| M3 | 规则可测化 | 规则→证据→检查点映射 | M2 |
| **M4** | **生成器 PoC** | **三命令最小闭环（compile/extract/trace）** | **M2** |
| M5 | 试点验证 | 真实模块评估报告 | M4 |
| M6 | 版本化推广 | CI 集成 + 升级/回滚手册 | M5 |

M0-M3 为设计阶段（已基本完成），**M4 是第一个产出可运行代码的里程碑**，以下为 M4 详细实施计划。
