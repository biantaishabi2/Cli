...
struct FileRecord {
    source_path: String,
    language: Language,          // elixir | go | rust | php | typescript
    loc_lines: usize,
    // 结构提取
    imports: Vec<ImportRecord>,  // 依赖引入
    exports: Vec<ExportRecord>,  // 公开接口（函数/类/trait/behaviour）
    declarations: Declarations,  // 各类声明计数
    calls: Vec<CallRecord>,     // 函数调用（含跨模块）
    // 副作用检测
    side_effects: SideEffects,  // async/await/spawn/网络/文件 IO
    // 文档
    module_doc: Option<String>, // 模块级文档注释
}
```
...
| **状态与副作用** | extract | `side_effects`（async/网络/文件 IO/spawn） | tree-sitter 检测 await/fetch/fs/spawn 等模式 |
...
> 全部 10 个步骤已完成，37/37 BDD 场景通过。实现代码位于 `compiler/bcc/src/`，测试 fixture 位于 `compiler/bcc/fixtures/`。

#### PoC 降级项（与设计目标的偏差）

当前 M4 实现以"最快跑通 BDD 黑盒场景"为目标，以下三处使用了降级方案，BDD 场景只验收了外部行为（输入/输出），未验收内部实现路径：

| # | 设计目标 | PoC 降级实现 | 影响 | 补齐条件 |
|---|---------|-------------|------|---------|
| D1 | extract 用 tree-sitter + `.scm` query 做 AST 级提取（§2.2.3, §13.5） | 用 `line.trim().starts_with()` 正则/字符串匹配 | 注释中的 `def` 会误匹配；无法提取 pattern match 参数、guard clause、宏展开；缺少 `declarations` 计数和 `loc_lines`；比已有 TS 原型（`audit-backend-ast.ts`，用 TS compiler API）和 PHP 原型（`audit_actions.php`，用 `token_get_all`）能力更弱 | 引入 `tree-sitter-elixir` + `tree-sitter-typescript` crate，编写 `.scm` query |
| D2 | compile emit 委托 Elixir（§13.0, §13.4）：Rust→JSON→`bcc_emit.exs`→`Macro.to_string`→`Code.format_string!`→.ex | `bcc_emit.exs` 未创建；`emit_simple()` 用 Rust 字符串拼接生成 .ex | 简单骨架能通过 `mix format`，但复杂 AST（嵌套 pattern match、with 链、管道）会生成不合法或格式错误的 Elixir 代码 | 创建 `scripts/bcc_emit.exs`（~30 行），CI 的 compile job 加 Elixir 环境 |
| D3 | extract 支持 3 种输出模式：`--mode ast`/`--mode doc`/`--mode yaml`（§2.2.3） | 只实现 `--mode ast` | 无法直接生成 per-file 分析文档或 YAML draft | 补齐 `--mode doc`（markdown 模板填充）和 `--mode yaml`（YAML draft 生成） |
...
> **⚠️ PoC 降级 D2**：`bcc_emit.exs` 未创建。实际走 `emit_simple()` Rust 字符串拼接。BDD 场景只验收了输出文件存在性和内容关键词，未验收 Elixir AST 往返一致性。
...
> **⚠️ PoC 降级 D1**：未引入 tree-sitter，用逐行 `starts_with`/`contains` 字符串匹配。缺少 `declarations` 计数、`loc_lines` 字段。BDD 场景只验收简单 fixture 的提取结果，未测试注释误匹配、多行 def、宏展开等边界情况。
...
> **⚠️ PoC 降级 D1**：同 Step 6，用字符串匹配替代 tree-sitter。比已有 `audit-backend-ast.ts` 原型（用 TS compiler API）能力更弱——缺少 declaration 分类计数、local dependency resolve、calleeImport 关联。
...
THEN assert_json_path path="$.exports[0].signature" expected_contains="{:get, key}"
...
THEN assert_json_path path="$.declarations.functionCount" expected_type=number

[SCENARIO: BCC-EXTRACT-106] TITLE: TS 提取包含 declaration 分类计数 TAGS: extract typescript completeness d1
LET source="fixtures/extract_edge/rich_module.ts"
# fixture 包含 export function(×2)/class/interface/type/enum/const
WHEN run_cli args="extract $source --mode ast"
THEN assert_json_path path="$.declarations.functionCount" expected=2
THEN assert_json_path path="$.declarations.classCount" expected=1
THEN assert_json_path path="$.declarations.interfaceCount" expected=1

[SCENARIO: BCC-EXTRACT-107] TITLE: TS 提取能解析 import binding 细节 TAGS: extract typescript completeness d1
LET source="fixtures/extract_edge/complex_imports.ts"
# fixture: import { foo as bar } from './mod'; import * as ns from 'pkg'
WHEN run_cli args="extract $source --mode ast"
THEN assert_json_path path="$.imports[0].bindings[0].kind" expected="named"
THEN assert_json_path path="$.imports[0].bindings[0].alias" expected="bar"
THEN assert_json_path path="$.imports[1].bindings[0].kind" expected="namespace"
```
...
WHEN run_shell cmd="elixir -e '
  ...
  {:ok, _} = Code.format_string!(roundtrip)
...
THEN assert_stdout_contains expected="emit via elixir"
THEN assert_stdout_not_contains expected="fallback to simple emit"

[SCENARIO: BCC-EMIT-104] TITLE: bcc_emit.exs 脚本存在且可执行 TAGS: emit prerequisite d2
WHEN run_shell cmd="test -f scripts/bcc_emit.exs"
THEN assert_exit_code expected=0
WHEN run_shell cmd="elixir scripts/bcc_emit.exs --help"
THEN assert_exit_code expected=0
```
...
THEN assert_stdout_contains expected="<!-- TODO: agent 补写 -->"
...
| 降级项 | 新增场景数 | 覆盖范围 | 状态 |
|--------|:--------:|---------|------|
| D1 extract tree-sitter | 7 | 注释误匹配/多行def/宏过滤/字符串误匹配/FileRecord完整性/TS declaration计数/TS import binding | 待实现 |
| D2 emit Elixir | 4 | AST 往返一致/pass 组合 AST 合法/实现路径验证/脚本存在性 | 待实现 |
| D3 extract 输出模式 | 3 | --mode doc 模板/--mode yaml 格式/yaml→compile 回环 | 待实现 |
| **合计** | **14** | | **待实现** |
...
3. 收集 PoC 反馈，识别 extract 精度不足的场景（复杂 pattern match、宏展开等）。
4. 评估是否需要引入 tree-sitter 替代当前正则提取（当前 PoC 用正则，满足 80% 场景）。
