...
| **M4** | **生成器核心** | **三命令最小闭环（compile/extract/trace）** | **M2** | **✅ 51/51 场景通过（37 核心 + 14 降级补齐）** |
| **M5** | **扩展：PHP extract + bugfix** | **四命令（+bugfix 四步流水线）+ 多语言支持** | **M4** | **✅ 69/69 场景通过（+18 新增）** |
| M6 | 版本化推广 | CI 集成 + 升级/回滚手册 | M5 | 待启动 |

M0-M3 为设计阶段（已完成），M4 核心三命令已实现（51 个 BDD 场景通过），**M5 已完成**：新增 PHP extract + bugfix 四步流水线 + 多语言支持（php/elixir/typescript），69/69 场景全部通过。下一步为 M6 版本化推广。

### 16.0.1 M6 测试矩阵（arch/bdd 新链路）

目标：把新增 `bcc arch` / `bcc bdd seed` 纳入可执行测试，且与双链路（Greenfield/Brownfield）对齐。

| 维度 | Greenfield（新项目主线） | Brownfield（存量补强线） | 当前状态 |
|---|---|---|---|
| 主流程可执行 | `compile -> arch matrix -> arch validate -> bdd seed` | `extract -> arch validate -> export-module-map -> bugfix` | ✅ 命令已实现 |
| 产物结构对齐 | `contexts/scenarios/features/coverage.md` | `contexts/scenarios/features/coverage.md` | ✅ 已对齐 |
| 门禁返回码 | `arch validate` 严格模式/报告模式 | `arch validate` 严格模式/报告模式 | ✅ 已支持（可切换） |
| 模块映射兼容 | `arch export-module-map`（可选） | `bugfix --module-map` 直接消费 | ✅ 已打通 |
| 自动化回归 | CLI 集成测试（arch/bdd） | CLI 集成测试（extract+bugfix+arch） | 🟡 已有基础 smoke，用例待扩展到 parity |
| TS/Rust parity | 产物字段一致性校验 | 产物字段一致性校验 | ❌ 待实现 |
| 生成质量回检 | `bdd seed generate/check/fix` 闭环 | `bugfix generate/check/fix` 闭环 | ❌ 待实现（seed 仍为模板替换） |

M6 阶段强制对齐项：
- 双链路都必须产出同口径场景包目录。
- `arch validate` 返回码策略必须可配置并有测试覆盖。
- `module_map` 输出必须保持 `bugfix` 兼容。

本轮已落地的基础自动化：
- 单测：`compiler/bcc/src/arch.rs`、`compiler/bcc/src/bdd_seed.rs` 新增链路与返回码测试。
- CLI 集成测试：`compiler/bcc/tests/cli_arch_bdd.rs` 覆盖 Greenfield/Brownfield smoke。
...
> M4 核心：37/37 BDD 场景通过。M5 降级补齐：D1/D2/D3 全部完成（14 个补充 BDD 场景通过）。实现代码位于 `compiler/bcc/src/`，测试 fixture 位于 `compiler/bcc/fixtures/`。

#### M4 降级项（与设计目标的偏差）— ✅ 已全部补齐

M4 阶段有 3 项降级，已在 M5 全部补齐：
...
```
[SCENARIO: BCC-SCAFFOLD-001] TITLE: bcc --help 显示四个子命令 TAGS: cli scaffold
...
THEN assert_stdout_contains expected="bugfix"
...
- 四个子命令：`compile`、`extract`、`trace`、`bugfix`
...
**Step 7b: bcc extract — PHP 支持**

输入：`.php` 源文件
输出：`FileRecord` JSON（与 Elixir/TS 同结构）

```
[SCENARIO: BCC-EXTRACT-PHP-001] TITLE: 提取 PHP public method 列表 TAGS: extract php
LET source="fixtures/sample_controller.php"
WHEN run_cli args="extract $source --mode ast"
THEN assert_json_path path="$.exports[0].name" expected="attentionnewsAction"
THEN assert_json_path path="$.exports[0].kind" expected="method"
THEN assert_json_path path="$.language" expected="php"

[SCENARIO: BCC-EXTRACT-PHP-002] TITLE: 忽略 private/protected 方法 TAGS: extract php
LET source="fixtures/sample_controller.php"
WHEN run_cli args="extract $source --mode ast"
THEN assert_json_array_not_contains path="$.exports[*].name" value="_formatUser"
THEN assert_json_array_not_contains path="$.exports[*].name" value="initialize"

[SCENARIO: BCC-EXTRACT-PHP-003] TITLE: 提取静态方法调用为 calls TAGS: extract php
LET source="fixtures/sample_controller.php"
WHEN run_cli args="extract $source --mode ast"
THEN assert_json_array_contains path="$.calls[*].callee" value="SysMsg"
THEN assert_json_array_contains path="$.calls[*].callee" value="OaComment"

[SCENARIO: BCC-EXTRACT-PHP-004] TITLE: 提取 class 继承和 trait use TAGS: extract php
LET source="fixtures/sample_controller.php"
WHEN run_cli args="extract $source --mode ast"
THEN assert_json_array_contains path="$.imports[*].specifier" value="MsbcArticleTrait"
THEN assert_json_path path="$.module_doc" expected_contains="OauserController"

[SCENARIO: BCC-EXTRACT-PHP-005] TITLE: PHP 自动语言检测 TAGS: extract php language-detect
LET source="fixtures/sample_controller.php"
WHEN run_cli args="extract $source --mode ast"
THEN assert_json_path path="$.language" expected="php"
...
实现要点：
- `tree-sitter-php` crate（`Cargo.toml` 新增 `tree-sitter-php = "0.24"`）
- PHP 特殊处理：`xxxAction()` 模式（Phalcon 约定）、`extends`、`use Trait`、`Class::method()` 静态调用
- 详见 [BDD场景提取方案.md](./BDD场景提取方案.md) §4.1
...
**Step 9: bcc bugfix — Git Bugfix → BDD 场景提取**

输入：Git 仓库路径
输出：bddc DSL 场景文件 + 覆盖率报告
详见：[BDD场景提取方案.md](./BDD场景提取方案.md)

```
[SCENARIO: BCC-BUGFIX-001] TITLE: bcc bugfix --help 显示用法和参数 TAGS: bugfix cli help
WHEN run_cli args="bugfix --help"
THEN assert_stdout_contains expected="Mine BDD scenarios from git bugfix history"
THEN assert_stdout_contains expected="--step"
THEN assert_stdout_contains expected="--grade"
THEN assert_stdout_contains expected="--keywords"
THEN assert_stdout_contains expected="--module-map"
THEN assert_stdout_contains expected="--output"
THEN assert_stdout_contains expected="--force"
THEN assert_exit_code expected=0

[SCENARIO: BCC-BUGFIX-002] TITLE: bugfix 缺少必填参数报错 TAGS: bugfix cli error
WHEN run_cli args="bugfix"
THEN assert_stderr_contains expected="<REPO>"
THEN assert_exit_code expected=2

[SCENARIO: BCC-BUGFIX-003] TITLE: bugfix --step collect 输出 inventory.json TAGS: bugfix collect
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_out"
WHEN run_cli args="bugfix $repo -o $out --step collect"
THEN assert_file_exists path="$out/inventory.json"
THEN assert_json_path file="$out/inventory.json" path="$.meta.repo" expected_contains="bugfix_repo"
THEN assert_json_path file="$out/inventory.json" path="$.meta.by_grade" expected_type="object"
THEN assert_exit_code expected=0

[SCENARIO: BCC-BUGFIX-004] TITLE: bugfix -s c 别名等效 --step collect TAGS: bugfix collect alias
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_alias"
WHEN run_cli args="bugfix $repo -o $out -s c"
THEN assert_file_exists path="$out/inventory.json"
THEN assert_exit_code expected=0

[SCENARIO: BCC-BUGFIX-005] TITLE: collect 按行数正确分级 TAGS: bugfix collect grading
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_grade"
WHEN run_cli args="bugfix $repo -o $out -s c"
THEN assert_json_array_contains file="$out/inventory.json" path="$.commits[*].grade" value="A"

[SCENARIO: BCC-BUGFIX-006] TITLE: collect 自动打标签匹配 commit message TAGS: bugfix collect tags
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_tags"
WHEN run_cli args="bugfix $repo -o $out -s c"
THEN assert_json_array_contains file="$out/inventory.json" path="$.commits[?(@.tags)].tags[*]" value="security"

[SCENARIO: BCC-BUGFIX-007] TITLE: --grade 筛选只保留指定级别 TAGS: bugfix collect filter
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_filter"
WHEN run_cli args="bugfix $repo -o $out -s c --grade A"
THEN assert_json_array_not_contains file="$out/inventory.json" path="$.commits[*].grade" value="C"

[SCENARIO: BCC-BUGFIX-008] TITLE: bugfix --step context 输出 context JSON TAGS: bugfix context
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_ctx"
WHEN run_cli args="bugfix $repo -o $out --step context"
THEN assert_file_exists path="$out/inventory.json"
THEN assert_dir_not_empty path="$out/contexts/"
THEN assert_exit_code expected=0

[SCENARIO: BCC-BUGFIX-009] TITLE: context 包含 before/after 函数体 TAGS: bugfix context content
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_ctx2"
WHEN run_cli args="bugfix $repo -o $out -s x"
THEN assert_json_path file="$out/contexts/*.json" path="$.diffs[0].hunks[0].function_name" expected_type="string"
THEN assert_json_path file="$out/contexts/*.json" path="$.diffs[0].hunks[0].before_function" expected_type="string"
THEN assert_json_path file="$out/contexts/*.json" path="$.diffs[0].hunks[0].after_function" expected_type="string"

[SCENARIO: BCC-BUGFIX-010] TITLE: context 断点续传跳过已有文件 TAGS: bugfix context resume
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_resume"
WHEN run_cli args="bugfix $repo -o $out -s x"
WHEN run_cli args="bugfix $repo -o $out -s x"
THEN assert_stderr_contains expected="skip"

[SCENARIO: BCC-BUGFIX-011] TITLE: --force 强制重做已有输出 TAGS: bugfix force
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_force"
WHEN run_cli args="bugfix $repo -o $out -s c"
WHEN run_cli args="bugfix $repo -o $out -s c --force"
THEN assert_stderr_not_contains expected="skip"
THEN assert_exit_code expected=0

[SCENARIO: BCC-BUGFIX-012] TITLE: --step 非法值报错 TAGS: bugfix cli error
LET repo="fixtures/bugfix_repo"
WHEN run_cli args="bugfix $repo -o tmp/ --step invalid"
THEN assert_stderr_contains expected="unknown step"
THEN assert_exit_code expected=1

[SCENARIO: BCC-BUGFIX-013] TITLE: --limit 限制处理的 commit 数 TAGS: bugfix collect limit
LET repo="fixtures/bugfix_repo"
LET out="tmp/bugfix_limit"
WHEN run_cli args="bugfix $repo -o $out -s c --limit 5"
THEN assert_json_array_length file="$out/inventory.json" path="$.commits" max=5
```
...
| 类别 | 场景数 | 覆盖范围 | 状态 |
|------|:------:|---------|------|
| M4 PoC（compile/extract/trace） | 37 | 基础三命令全闭环 | ✅ 已通过 |
...
| M5 extract PHP | 5 | public method/private过滤/静态调用/继承trait/语言检测 | ✅ 已通过 |
| M5 bugfix | 13 | help/参数校验/collect分级标签筛选/context函数体续传/force/limit/别名 | ✅ 已通过 |
| **合计** | **69** | | **69/69 通过** |
...
### 18.3 M5 扩展：Git Bugfix → BDD 场景提取

从 FIT365 的 ~3,700 个 bugfix commit 中提取 bddc DSL 场景，为 Phoenix 迁移提供回归覆盖。

新增 `bcc bugfix` 子命令（单命令，`--step` 控制执行阶段：collect/context/generate/organize），以及 `bcc extract` 的 PHP 语言扩展。

详见子文档：[BDD场景提取方案.md](./BDD场景提取方案.md)
