...
**场景**：已有 Elixir 模块，想反向生成 YAML 初稿。

`bcc extract <source_path>` 扫描现有代码，提取模块结构：
...
$ bcc extract lib/shop/order/order_service.ex
  ...
  Found:
    module: Shop.Order.OrderService → OrderService
    ports:  3 public functions → 3 ports (create_order, cancel_order, get_order)
    errors: 5 error atoms → 5 error codes (需人工确认命名)
    relations: 2 module calls → 2 relations (PaymentService, InventoryService)

  ⚠ Draft — 请人工审核后再用于编译
```

提取规则：
- **public 函数 → ports**：`def` 的函数签名提取为 port，`defp` 忽略。
- **@spec → input/output**：有 typespec 的提取参数类型和返回类型。
- **{:error, atom} → errors**：模式匹配 `{:error, :reason}` 提取为错误码候选。
- **Module.function() 调用 → relations**：跨模块调用提取为 callee 关系。
- **@moduledoc → responsibility**：模块文档第一句作为职责说明。

提取的 YAML 标记为 `# [EXTRACTED — 需人工确认]`，人工审核后删除标记即可用于编译。
...
- **tree-sitter-elixir**（Rust crate，可选）：对生成的 `.ex` 做语法验证
...
```
bcc/
├── target/release/bcc          # Rust 编译产物（单 binary）
└── scripts/bcc_emit.exs        # Elixir 序列化脚本（~30 行，随 binary 分发）
```

- **安装**：将 `bcc` 放入 `$PATH`，`bcc_emit.exs` 放在 binary 同级 `scripts/` 下（或嵌入 binary 作为资源文件运行时写出临时文件）。
- **运行时依赖**：仅要求目标机器有 `elixir` 命令可用——BCC 的使用场景是 Elixir/Phoenix 项目，此条件始终满足。
- **无 Elixir 环境降级**：`bcc` 检测不到 `elixir` 时报明确错误并退出，不做静默降级。
...
## 16. 分阶段计划（M0-M6）
1. M0 范围对齐：明确边界与接口点（含 stitch/bddc）。
2. M1 最佳实践候选：形成候选清单与冲突项。
3. M2 最小规范定稿：产出可执行规范 v1（必须/推荐/禁用）。
4. M3 规则可测化：建立规则 -> 证据 -> 检查点映射。
5. M4 生成器 PoC：单模块闭环验证（不追求全量）。
6. M5 试点验证：在真实模块评估效率/质量/稳定性。
7. M6 版本化推广：纳入 CI、升级策略与回滚手册。
...
### 18.2 当前下一步
1. 先收敛候选最佳实践，不直接进入全量代码生成。
2. 产出“候选清单 + 冲突项”，组织规则评审。
3. 评审通过后进入 M2，形成可执行规范 v1。
