...
- 详见 [测试规格书提取方案](./BDD场景提取方案-v2.md) §4.1
...
**Step 9: bcc bugfix — Git Bugfix → 测试规格书提取**

输入：Git 仓库路径
输出：测试规格书 JSON（specs/*.json → by_module/*.json）+ 覆盖率报告
详见：[测试规格书提取方案](./BDD场景提取方案-v2.md)
...
从 FIT365 的 ~3,700 个 bugfix commit 中提取测试规格书 JSON，为 Phoenix 迁移提供回归覆盖。

新增 `bcc bugfix` 子命令（单命令，`--step` 控制执行阶段：collect/context/generate/organize），以及 `bcc extract` 的 PHP 语言扩展。

详见子文档：[测试规格书提取方案](./BDD场景提取方案-v2.md)
