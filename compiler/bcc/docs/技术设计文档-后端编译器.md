...
| **M4** | **生成器 PoC** | **三命令最小闭环（compile/extract/trace）** | **M2** | **✅ 51/51 场景通过（37 PoC + 14 降级补齐）** |
| **M5** | **扩展：PHP extract + bugfix** | **四命令（+bugfix 四步流水线）+ 多语言支持** | **M4** | **✅ 69/69 场景通过（+18 新增）** |
| M6 | 版本化推广 | CI 集成 + 升级/回滚手册 | M5 | 待启动 |

M0-M3 为设计阶段（已完成），M4 PoC 已实现（51 个 BDD 场景通过），**M5 已完成**：新增 PHP extract + bugfix 四步流水线 + 多语言支持（php/elixir/typescript），69/69 场景全部通过。下一步为 M6 版本化推广。
