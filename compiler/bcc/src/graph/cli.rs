...
pub fn build_index(
    ...
    // 读取 extract 输出
    let ast_json = std::fs::read_to_string(extract_output).map_err(|e| GraphError::Io(e))?;

    let snapshot: crate::extract::AstSnapshot =
        serde_json::from_str(&ast_json).map_err(|e| GraphError::Serialization(e))?;

    println!(
        "[graph] Parsed AST snapshot: {} files",
    ...
    for record in &snapshot.records {
        ...
        for dep in &record.localDependencies {
            ...
            // 找到匹配的目标模块
            let matched_target = possible_targets
                .iter()
    ...
    for record in &snapshot.records {
        ...
        for target in &record.localCallTargets {
            ...
            // 找到匹配的函数 ID
            let matched_target = possible_targets.iter().find(|t| func_ids.contains(*t));
    ...
    println!(
        "[graph] Indexed {} modules, {} module deps, {} functions, {} call edges",
...
pub fn query_function(repo_id: &str, query: &str, query_type: QueryType) -> Result<()> {
    let manager = GraphStoreManager::default()?;
    ...
    match query_type {
        ...
        QueryType::Callers { depth } => match store.find_callers(query, depth) {
            Ok(callers) => {
                println!(
                    "Found {} callers of '{}' (depth={})",
                    callers.len(),
                    query,
                    depth
                );
        ...
        QueryType::Callees { depth } => match store.find_callees(query, depth) {
            Ok(callees) => {
                println!(
                    "Found {} callees of '{}' (depth={})",
                    callees.len(),
                    query,
                    depth
                );
        ...
        QueryType::Impact => match store.analyze_impact(&[query.to_string()]) {
            Ok(impact) => {
...
pub fn search_graph(repo_id: &str, function_id: &str, depth: usize, include: &str) -> Result<()> {
    let manager = GraphStoreManager::default()?;
    ...
    if include_types.is_empty() {
        return Err(GraphError::InvalidArgs(format!(
            "Invalid include types: {}. Valid: callers,callees,siblings,same-file,same-module",
            include
        )));
    }
    ...
    if !result.classes.is_empty() {
        ...
        for class in &result.classes {
            println!(
                "  - {} ({}:{})",
                class.name, class.file_path, class.start_line
            );
        }
    }
...
pub fn query_module(repo_id: &str, module_id: &str, query_type: ModuleQueryType) -> Result<()> {
    let manager = GraphStoreManager::default()?;
    ...
    match query_type {
        ...
        ModuleQueryType::Deps { depth } => match store.find_module_deps(module_id, depth) {
            Ok(deps) => {
                println!(
                    "Found {} dependencies of '{}' (depth={})",
                    deps.len(),
                    module_id,
                    depth
                );
        ...
        ModuleQueryType::Circular => match store.detect_circular_deps(module_id) {
            Ok(Some(cycle)) => {
...
pub fn validate_arch(repo_id: &str, target_path: &str, output_path: Option<&str>) -> Result<()> {
    let manager = GraphStoreManager::default()?;
