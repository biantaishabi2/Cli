	...
	if !blockedByPersisted {
		...
		if err != nil {
			...
			for _, task := range readyTasks {
				...
				if issueNum > 0 {
					if err := state.TransitionBotState(ctx, c.github, issueNum, state.StateQueued, state.StateFixRequested); err != nil {
						if errors.Is(err, state.ErrMultipleBotStates) {
							fmt.Printf("[control] action=progress_blocked issue_num=%d task_id=%s reason=dirty_multi_state err=%v\n", issueNum, task.ID, err)
							continue
						}
						fmt.Printf("[control] 替换标签失败 (issue #%d): %v\n", issueNum, err)
					} else {
