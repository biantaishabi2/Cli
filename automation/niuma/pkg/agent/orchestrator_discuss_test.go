...
func TestDoDiscuss_ConvergesAndFinalize(t *testing.T) {
	mockAI := ai.NewMockProvider(
		`{"agreements":["第一轮讨论"],"disagreements":[{"topic":"补充边界","options":["A","B"],"recommendation":"A","risk":"medium"}],"suggestion":"继续讨论"}`,
		`{"agreements":["已达成一致"],"disagreements":[],"suggestion":"可定稿"}`,
...
func TestDoDiscuss_StopsAtRoundLimit(t *testing.T) {
	mockAI := ai.NewMockProvider(
		`{"agreements":["r1"],"disagreements":[{"topic":"a","options":["A","B"],"recommendation":"A","risk":"medium"}],"suggestion":"继续"}`,
		`{"agreements":["r2"],"disagreements":[{"topic":"b","options":["A","B"],"recommendation":"A","risk":"medium"}],"suggestion":"继续"}`,
		`{"agreements":["r3"],"disagreements":[{"topic":"c","options":["A","B"],"recommendation":"A","risk":"medium"}],"suggestion":"继续"}`,
	)
	mockGH := NewMockGitHub()
	mockGH.SetIssue(1, "Fix login", "Body")
	mockGH.SetLabel(1, string(state.StateNeedsDiscussion))
...
func TestDoDiscuss_RoundLimitNoticeDoesNotAutoFinalize(t *testing.T) {
	mockAI := ai.NewMockProvider(
		`{"agreements":["r1"],"disagreements":[{"topic":"a","options":["A","B"],"recommendation":"A","risk":"medium"}],"suggestion":"继续"}`,
		`{"agreements":["继续讨论"],"disagreements":[{"topic":"b","options":["A","B"],"recommendation":"A","risk":"medium"}],"suggestion":"继续"}`,
		`{"title":"最终方案","approach":"按共识实现","file_changes":[{"path":"src/login.go","action":"modify","description":"修复编码"}],"test_scenarios":[{"name":"特殊字符","input":"p@ss","expected":"success"}]}`,
	)
	mockGH := NewMockGitHub()
	mockGH.SetIssue(1, "Fix login", "Body")
...
func TestDoDiscussionCheck_StaleConvergeWarningDoesNotAutoFinalize(t *testing.T) {
	mockAI := ai.NewMockProvider(
		`{"agreements":["继续讨论"],"disagreements":[{"topic":"补充约束","options":["A","B"],"recommendation":"A","risk":"medium"}],"suggestion":"继续"}`,
	)
	mockGH := NewMockGitHub()
	mockGH.SetIssue(1, "Fix login", "Body")
	mockGH.SetLabel(1, string(state.StateNeedsDiscussion))
...
func TestDoDiscuss_ConcurrentDuplicateTriggers_StateRemainsStable(t *testing.T) {
	...
	mockAI := ai.NewMockProvider(
		`{"agreements":["继续讨论"],"disagreements":[{"topic":"a","options":["A","B"],"recommendation":"A","risk":"medium"}],"suggestion":"继续"}`,
		`{"agreements":["继续讨论"],"disagreements":[{"topic":"b","options":["A","B"],"recommendation":"A","risk":"medium"}],"suggestion":"继续"}`,
	)
	mockGH := NewMockGitHub()
	mockGH.SetIssue(1, "Fix login", "Body")
